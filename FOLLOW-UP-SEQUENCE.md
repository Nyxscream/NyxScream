# NYXSCREAM - FOLLOW-UP SEQUENCE

**The difference between funded and ghosted is persistence**
**Follow up strategically, not desperately**

---

## 🎯 THE FOLLOW-UP FRAMEWORK

**Rule 1:** If they don't respond in 3 days, follow up
**Rule 2:** Never ask "Did you get my email?" (they did)
**Rule 3:** Always give them a new reason to engage
**Rule 4:** Track everything (who you've reached out to, when, response)
**Rule 5:** After 3 follow-ups with no response = move on

---

## 📅 COLD EMAIL FOLLOW-UP SEQUENCE (Days 1-21)

### **DAY 0: SEND INITIAL COLD EMAIL**

[Use cold email template from Document 2]

**Timing:** Tuesday-Thursday, 9-10am their timezone
**Goal:** Get initial response

---

### **DAY 3: FIRST FOLLOW-UP (Soft)**

**Subject Line:** Re: [Original subject line]

**Email:**

Hi [First Name],

Just wanted to check in on my previous email. No pressure if this isn't the right timing.

Quick context: We're building NyxScream, a creator-first streaming platform that pays creators 70% (vs Twitch's 50%).

I saw you led the Series A for [Company]—thought the mission alignment might resonate.

If you're curious, I can give you a 15-min demo. No strings attached.

Otherwise, would you know anyone who should see this?

Thanks,
Isaac

P.S. - If you want to check out the code: github.com/Nyxscream/NyxScream

---

**Why this works:**
✅ Acknowledges they're busy
✅ Gives new reason to engage (GitHub link)
✅ Asks for intro (lower ask if not interested)
✅ Soft, not pushy

---

### **DAY 7: SECOND FOLLOW-UP (Add Value)**

**Subject Line:** One thing I thought you'd care about

**Email:**

Hi [First Name],

Real quick—I realized you backed [Company], which raised $[X] at valuation $[Y].

Our unit economics are similar: $2-5 CAC, $500+ LTV, 100:1 ratio.

But here's what's different: We're profitable by Month 6, not chasing growth-at-all-costs.

I think you'd find the financial model interesting.

Open to a 10-min call to walk through it?

Isaac

---

**Why this works:**
✅ Shows you researched them (not generic)
✅ Speaks their language (unit economics)
✅ Shorter ask (10 min, not 15)
✅ Positions as value-add

---

### **DAY 10: THIRD FOLLOW-UP (Social Proof)**

**Subject Line:** Just hit [milestone]

**Email:**

Hi [First Name],

Quick update: We just hit [milestone].

[Pick one of:]
- Launched on Product Hunt (250+ upvotes)
- 50 creators signed up in first week
- Got intro to [other investor who's interested]
- GitHub hit 100 stars

Thought you'd want to know since you track momentum in this space.

If this is still on your radar, let's grab a call this week.

Isaac

---

**Why this works:**
✅ Shows momentum (reduces risk perception)
✅ Creates FOMO (others are interested)
✅ Gives update (not just nagging)
✅ Specific ask (call this week)

---

### **DAY 14: FINAL ATTEMPT (Direct)**

**Subject Line:** Last one—promise

**Email:**

Hi [First Name],

I've reached out a few times. Either this isn't the right fit, or you're swamped.

Either way, I get it.

But if you're even 10% interested, I'd rather get a 5-min "no thanks" call than wonder.

Here's what I'd want to discuss:
1. Is the mission alignment real?
2. Is the team (me) credible?
3. Are the numbers realistic?

If you have concerns on any of these, I'd love to address them.

If not—completely understand. And if you know anyone who should see this, I'd appreciate an intro.

Thanks for considering.

Isaac
[Phone number]

---

**Why this works:**
✅ Acknowledges the silence (respects their time)
✅ Makes it easy to say no (low pressure)
✅ Specific asks (not vague)
✅ Gives phone number (shows confidence)

---

### **DAY 21: MOVE ON**

No response after 4 touchpoints = Move on.

**Log them as:**
- "Not interested (no response)"
- Try again in 6 months when you have traction
- Don't waste emotional energy

---

## 🔄 WARM INTRO FOLLOW-UP SEQUENCE

**[Different from cold emails]**

### **DAY 0: MUTUAL FRIEND SENDS INTRO**

Your friend emails investor:

"[Investor], meet Isaac. He's building NyxScream, a creator-first streaming platform (70/30 split vs Twitch's 50/50).

Production-ready app, profitable in Year 1, raising $1M.

I think you should take a meeting. Isaac, [Investor] knows this space intimately.

[Investor], check out the GitHub: github.com/Nyxscream/NyxScream"

---

### **DAY 1: YOU FOLLOW UP WITH INVESTOR**

**Subject:** [Friend's name] intro - quick chat?

**Email:**

Hi [Investor Name],

[Friend] just connected us. Thanks for making time.

I'm building NyxScream—creator-first streaming with 70/30 split. I know you back creators, so thought this might interest you.

Are you open to a 20-min call this week? I can show you the app + run through the numbers.

No pressure if the timing isn't right.

Isaac
[Phone number]
github.com/Nyxscream/NyxScream

---

**Response rates with warm intros:**
- Day 1-2: 30-40% respond
- Day 3-7: 10-20% respond
- After Day 7: <5% respond

---

### **DAY 3: FOLLOW-UP (If No Response)**

"Hi [Name],

Just want to make sure [Friend]'s email came through. If you're swamped, I totally get it.

Otherwise, would love to chat. Free tomorrow or Thursday afternoon (your timezone)?"

---

### **DAY 7: FINAL FOLLOW-UP**

"[Name],

Taking your silence as a no. All good—appreciate the consideration anyway.

If anything changes, I'm here. And if you know anyone else who should see this, I'd be grateful for an intro."

---

## 📱 TWITTER/LINKEDIN FOLLOW-UP SEQUENCE

**[After initial DM]**

### **DAY 1: INITIAL DM**

[Use Twitter DM template from Document 2]

---

### **DAY 3: ENGAGEMENT**

If no response to DM:
- Like their recent tweet
- Comment thoughtfully on their post
- Don't mention NyxScream (not pushy)

**Goal:** Get back on their radar without asking

---

### **DAY 5: SECOND DM**

"Hey [Name], saw your tweet about [topic]. Totally agree on [opinion].

By the way, if you ever want to chat about creator platforms, I'm building something interesting. No pressure though!"

---

### **DAY 10: MOVE ON OR PIVOT**

If still no response:
- Stop DMing (you've tried)
- Keep engaging with their content (build relationship)
- Try email or warm intro instead

---

## 🎯 MULTI-CHANNEL FOLLOW-UP (Advanced)

**If email isn't working, try multiple channels:**

### **Channel 1: Email**
- Day 0: Send
- Day 3: Follow-up
- Day 7: Follow-up
- Day 14: Final

### **Channel 2: Twitter/LinkedIn**
- Day 2: Engage with content
- Day 5: DM
- Day 10: Mention in DM

### **Channel 3: Warm Intro**
- Day 4: Ask mutual friend for intro
- If they send intro, restart sequence

### **Channel 4: Physical Mail**
- Day 8: Send printed pitch deck + personal note
- Creates differentiation
- Costs $5 per investor
- 2-3x response rate vs email alone

---

## 📊 FOLLOW-UP TRACKING SPREADSHEET

**Create a spreadsheet to track:**

| Investor Name | Fund | Email | Day 1 | Day 3 | Day 7 | Day 14 | Status | Notes |
|---|---|---|---|---|---|---|---|---|
| [Name] | [Fund] | [Email] | ✅ Sent | ⏳ No response | ✅ Sent | ⏳ Waiting | Warm intro pending | Follow up in 2 weeks |
| [Name 2] | [Fund 2] | [Email] | ✅ Sent | ✅ Responded | 📞 Call scheduled | - | Demo meeting | Thursday 2pm |

**Columns:**
- Investor Name
- Fund name
- Their email
- Day 1 status (sent, response, etc.)
- Day 3 status
- Day 7 status
- Day 14 status
- Overall status (interested/ghosted/moved on/meeting scheduled)
- Notes (why they might fund you, intro needed, etc.)

---

## 🎁 WHAT TO SEND IN FOLLOW-UPS

**Emails are good, but attachments are better:**

### **Send with Follow-up #2:**
- 1-page business summary
- 3-slide pitch deck (problem/solution/ask)
- GitHub link

### **Send with Follow-up #3:**
- Financial model (spreadsheet)
- Product demo link (Loom video)
- Testimonial from test creator

### **Send by Day 14:**
- Full pitch deck (20 slides)
- Terms sheet (what you're raising)
- Legal docs (cap table, equity breakdown)

---

## 🚀 SPECIAL FOLLOW-UP TACTICS

### **Tactic 1: The "Social Proof" Update**

When you hit any milestone, email all prospects:

"Quick update for everyone I've been talking to:

We just [hit milestone]. Sharing because momentum matters.

If you were on the fence, this might move the needle. If you're interested, let's talk this week."

**When to use:** After Product Hunt launch, 100 creators, press mention

---

### **Tactic 2: The "Other Investor" Mention**

"A few investors are already reviewing. If you want to move forward, now's the time. Happy to share a copy of the investment terms."

**When to use:** After 2+ serious investors show interest
**Caution:** Only use if true (they'll verify)

---

### **Tactic 3: The "Founder Update"**

Every 2 weeks, email all interested investors:

"Here's what we shipped this week:
- [Feature 1]
- [Metric 1]
- [Update 1]

How can we help if you're considering investing?"

**When to use:** Keep them warm between meetings

---

### **Tactic 4: The "Ask for Advice" Follow-up**

"I'm getting feedback that [concern]. What's your take on this?"

**Why:** Gives them a reason to engage that's not about money

---

### **Tactic 5: The "Partner Introduction" Follow-up**

"I'm talking to [relevant company] about a partnership. Thought you'd want to know since it relates to our thesis."

**Why:** Shows momentum without explicitly asking for money

---

## 🎯 TIMING STRATEGY

**Best days to follow up:**
✅ Tuesday (inbox reset after Monday)
✅ Wednesday (mid-week action)
✅ Thursday (end of week before they disappear)

**Best times to follow up:**
✅ 9-10am (morning check-in)
✅ 2-3pm (post-lunch energy)

**Worst times:**
❌ Monday (email overload)
❌ Friday (end of week, checking out)
❌ Midnight-7am (unprofessional)
❌ 12-1pm (eating)
❌ 5pm+ (done for the day)

---

## 📈 FOLLOW-UP METRICS

**Track your success:**

| Metric | Goal | How to Calculate |
|---|---|---|
| Email open rate | 30%+ | [Opens] / [Sent] |
| Click-through rate | 5%+ | [Clicks] / [Opened] |
| Response rate | 10%+ | [Responded] / [Sent] |
| Meeting rate | 5%+ of responses | [Meetings] / [Responded] |
| Close rate | 30-50% of meetings | [Funded] / [Meetings] |

**Example:**
- 50 emails sent
- 15 opened (30% open rate) ✅
- 1 clicked link (6% CTR) ✅
- 5 responded (33% response rate) ✅
- 1 meeting booked (20% meeting rate) ✅
- If they fund = 100% close rate ✅

---

## ❌ FOLLOW-UP MISTAKES TO AVOID

❌ **Mistake 1: Same message every time**
"Did you get my email?" = Delete
Use new angle each time

❌ **Mistake 2: Waiting too long to follow up**
"I'll wait a week before following up" = Too late
Follow up Day 3

❌ **Mistake 3: Following up too much**
10+ emails = Spam
Max 4 before moving on

❌ **Mistake 4: Sounding desperate**
"Really hope you respond" = Red flag
Sound confident always

❌ **Mistake 5: Negative follow-up**
"I guess you're not interested" = Petty
Stay positive always

❌ **Mistake 6: Not tracking**
"Did I already email them?" = Looks bad
Spreadsheet everything

❌ **Mistake 7: Generic attachments**
Same doc to 50 people = Obvious
Personalize each one

---

## ✅ FOLLOW-UP BEST PRACTICES

✅ **Every follow-up has a new angle**
✅ **Always give them a reason to respond**
✅ **Keep messages short (2-3 paragraphs)**
✅ **Make next step crystal clear**
✅ **Track everything in spreadsheet**
✅ **Respect their time (max 4 follow-ups)**
✅ **Move on after Day 21 with grace**
✅ **Revisit with traction in 6 months**

---

## 🎬 SAMPLE 21-DAY FOLLOW-UP SEQUENCE

**Investor: [Name], Fund: [Fund]**

**Day 0:** Send cold email
"I'm building NyxScream. Creators 70%, real transparency, profitable in Year 1."

**Day 3:** First follow-up
"Checking in. GitHub: github.com/Nyxscream"

**Day 7:** Value-add follow-up
"Our unit economics: 100:1 LTV/CAC. Profitable by Month 6."

**Day 10:** Social proof follow-up
"Just hit 100 creators. Momentum building."

**Day 14:** Direct ask
"Are you interested or should I move on?"

**Day 21:** Move on gracefully
"No response = move on. Try again in 6 months when we have 10K creators."

---

## 🏁 FOLLOW-UP CLOSES

**Different ways to end follow-ups:**

**Soft close:**
"Let me know if you want to chat."

**Hard close:**
"Are you in or out? Just need clarity."

**Question close:**
"What would make this a yes for you?"

**Alternative close:**
"Not interested in investing? Would you refer someone?"

**Specific close:**
"Free Tuesday or Thursday? Let's grab 15 min."

---

**NyxScream Follow-Up Mastery**
**Persistence Without Desperation**

🌑✨💜